...
class OurArguments(TrainingArguments):
    ...
    # Pissa
    pissa: bool = False # whether to use LoRA
    ...
    tag: str = "" # saving tag
    curve_path: str = field(default=None, metadata={"help": "Path to save convergence curve image"}) 
...
def get_model_weights_memory(model):
    ...
    for name, param in model.named_parameters():
        ...
        # print(f"Memory: {param.numel() * param.element_size() / 1024**2:.2f} MB")
...
def get_activation_memory():
    ...
    return memory_allocated / (1024 ** 2), memory_cached / (1024 ** 2)  # 转换为 MB
...
class Framework:
    ...
    def load_model(self):
        ...
        with count_time("Loading model with FP%d" % (16 if self.args.load_float16 else 32)):
            ...
            #config = AutoConfig.from_pretrained("microsoft/phi-1")
            ...
            # elif self.args.pissa:
            #     residual_model = AutoModelForCausalLM.from_pretrained(self.args.model_name, device_map="auto")
            #     model = PeftModel.from_pretrained(residual_model, self.args.model_name, subfolder = "pissa_init", is_trainable=True)
            else:
                # Auto device loading
            ...
            print(f"Model weights memory: {get_model_weights_memory(model)} MB")
            model.eval()
        ...
        print("self.args.model_name:", self.args.model_name)
        ...
        if self.args.lora:
            ...
            LoRA(model, r=self.args.lora_r, alpha=self.args.lora_alpha, float16=self.args.load_float16, bfloat16=self.args.load_bfloat16, pissa=self.args.pissa)
        if self.args.pissa:
            from lora import LoRA
            LoRA(model, r=self.args.lora_r, alpha=self.args.lora_alpha, float16=self.args.load_float16, bfloat16=self.args.load_bfloat16, pissa=self.args.pissa)
